...
public:
	...
	WordsMultiset getCommonWords(const WordsMultiset& other) const;
	WordsMultiset getUniqueWords(const WordsMultiset& other) const;

	void printMultiset() const;
	size_t getSize();

	private:
		HashMap<std::string, size_t> hashMap;
};
...
public:
	...
	// You can add additional members if you need to
	WordsMultiset readStream(std::istream& a);
	WordsMultiset getCommonWords(const WordsMultiset& m1, const WordsMultiset& m2);
	WordsMultiset getUniqueWords(const WordsMultiset& m1, const WordsMultiset& common);

	//ONLY for console version
	//void printConsoleOutput(WordsMultiset& m1, WordsMultiset& m2, WordsMultiset& common);
