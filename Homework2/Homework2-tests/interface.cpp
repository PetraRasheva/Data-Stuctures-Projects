#include "interface.h"
#include "../Homework2-Application/Tree.hpp"
#include <iostream>
...
//Трябва да може да инкорпорирате една йерархия. Това става като за всеки екип (множество служители с общ пряк ръководител)
//, който има поне двама служителя, се избере служителят с най-висока заплата и се направи шеф на този екип. Ако този служител 
//има собствени подчинени, те остават в неговия екип. Ако има повече от един такъв служител, да се избере този с лексикографски
//най-малкото име. Инкорпорирането започва от най-ниските нива в йерархията. На фигурата по-долу е показана йерархия преди 
//инкорпориране и съответно след. В първоначалната йерархия са подчертани със зелено екипите и с червено служителят, 
...
void Hierarchy::incorporate(TreeNode* root)
{
	...
	if (root->children.size() >= 2)
	{
		...
		//обхожд. децата на child и всички без newBoss ги преместваме да са деца на newBoss
...
Hierarchy Hierarchy::join(const Hierarchy& right) const
{
	...
	// TO DO
	/*BFS(root, right, 0);
	for (auto node : right.root->children)
		root->children.push_back(node);*/
}

void Hierarchy::BFS(TreeNode* root,const Hierarchy& right, int height) const
{
	//End
	if (!root) return;
	...
	TreeNode* duplicate = findDuplicate(root->name, duplicate_height, right.root); //TODO findDuplicate();
	if (duplicate)
	{
		if (height < duplicate_height)
			for (TreeNode* child : duplicate->children)
				child->children.push_back(child);
		else if (duplicate_height < height)
			for (TreeNode* child : root->children)
				duplicate->children.push_back(child);
		else if (height == duplicate_height)
			//TODO
			for (TreeNode* child : duplicate->children)
				child->children.push_back(child);
	}
